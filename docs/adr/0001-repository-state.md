...
- Raw diff fixtures are only for parser tests. They cannot describe full Git state.
...
Events mean "refresh." Do not try to turn each filesystem event into a Git change.
Discard an old refresh result if a newer refresh started.
...
3. Parse real Git state and test it end to end. See [ADR 0004](0004-real-git-state.md).
