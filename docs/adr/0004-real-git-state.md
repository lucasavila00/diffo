...
The `diffo-e2e` crate uses Insta snapshot tests. It builds and runs the real `diffo`
binary. Tests:
...
`DIFFO_DUMP_PATH` makes Diffo write one RON snapshot and exit before starting the TUI.
It is only for debugging and E2E tests. Diffo still has no command-line arguments.
