...
fn uncached_scroll_uses_one_viewport_and_skeleton_until_syntax_is_ready() {
    ...
    for (line, row) in skeleton.iter().zip(rows) {
        assert_eq!(line.spans[0].style, crate::style::gutter_style(row.kind));
    }
...
fn hunk_markers_have_a_separate_clickable_rail_beside_the_scrollbar() {
    ...
    assert_eq!(
        ...
        Some(crate::ScrollbarAxis::Vertical)
    );
    assert_eq!(
...
fn large_hunk_buttons_are_fixed_and_do_not_wrap() {
    ...
    let content_and_horizontal_bottom = |viewport: crate::DiffViewportMetrics| {
        viewport
            .content_area
            .bottom()
