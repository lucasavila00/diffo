...
structural and interactive styles, a shared Nerd Font icon vocabulary, and semantic
layout tokens for widths, heights, insets, gaps, and overlay bounds. It also owns the
fixed file-icon table, shared command palette, reusable searchable modal picker, file
picker, and read-only text view. Diffo requires a terminal font with Nerd Font
symbols. Callers own typed picker items, command execution, file activation, content
loading, and activity state.
