...
Read files on the existing bounded pool while one additional worker reads
metadata. Join both before publishing the snapshot. Do not run watch-path
discovery beside Git status; measurements showed that made one-file startup much
slower.
