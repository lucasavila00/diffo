...
Do not use theme bold, italic, or underline. Terminal fonts can make these too
loud. Use syntax foreground colors only.
...
Monokai expects a dark plain background. Some colors, such as comments, are hard
to read on red or green.

Keep a syntax color when its contrast is at least 4.5:1. Otherwise lighten it
until it reaches 4.5:1. Do this only on added and removed rows. Do not change
context-row colors.
