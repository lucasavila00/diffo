...
The repository footer identifies a named branch but not the exact revision
checked out on that branch. Branch names can point to different commits across
repositories and can move after refreshes and Git operations, so the branch
alone is insufficient when a user needs to confirm the revision being reviewed.

Detached HEAD already displays an abbreviated commit hash. Named HEAD snapshots
also contain the commit hash, but the footer discards it.

## Decision

Display the seven-character abbreviated `HEAD` commit hash after every named
branch:
...
and file state. Do not run Git from the renderer or introduce another cache.
This keeps the branch and hash atomic across refreshes and repository
operations.
...
the combined branch-and-hash text with an ellipsis.

## Alternatives

- Show the full commit hash. Rejected because it consumes too much of the footer
  and competes with repository state, operation feedback, and command help.
- Show the hash only for detached HEAD. Rejected because it leaves named
