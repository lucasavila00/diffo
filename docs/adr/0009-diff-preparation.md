...
- Render only visible rows. Build the active view projection and cache up to four
  recently prepared file-and-mode buffers; prepare the other mode only when asked.
- Syntax-highlight files below 10,000 lines using the bounded viewport windows from
  ADR 0032. File line count comes from parsed old/new line numbers, independent of
  patch byte size and the synchronous-work threshold. Skip syntax highlighting at
  10,000 lines and above.
...
- Syntax work is bounded by viewport size and parser look-behind, not file length.
