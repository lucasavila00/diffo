...
synthesize Explorer entries from changed paths in the shared Diff snapshot. Removed
paths belong to Diff and do not appear in Explorer. Do not add unchanged paths or
file contents to the shared Diff snapshot.
...
background. If a selected path disappears before it is read, discard the result and
refresh the tree; do not substitute its HEAD content. Binary or invalid UTF-8 files
...
mapping, removal of deleted paths and deletion-only folders, binary files, stale
results, and atomic syntax-ready opens.
