...
Sync still stops for detached or unborn HEAD, an active merge/rebase/cherry-pick,
local merge commits that would need rebasing, rebase conflicts, and rejected pushes.

## Result

Users can Sync with unrelated local edits. Users can also Sync a branch immediately
after a devcontainer rebuild, even when its upstream configuration was lost.

Covered by Real-Git, state-transition, and frame-traced PTY tests. `make all` passes.
